<script lang="ts">
	...
	/* NOTE
			The underlying rule in Svelte 5 is that destructured props cannot 
			be used directly in raw JavaScript statements at the root of your 
			script tag, because those statements only execute once during 
			component initialization.
			script module executes exactly once when your application first imports 
			the file. It is used for shared constants, utilities, or caching data 
	...
	const labelDown = `opacity:0.5;top:${height / 3}px;z-index:10;`;
	...
	(function () {
		// IIFE
		exportValueOn = (exportValueOn as TExportValueOn).toLowerCase() as TExportValueOn;
	...
	const topPosition = `${-1 * Math.floor(parseInt(fontsize as string) / 3)}px`;
	...
	const onBlurHandler = (event: FocusEvent) => {
		...
		if ((exportValueOn as TExportValueOn).includes('blur')) {
	...
	const onKeyUpHandler = (event: KeyboardEvent) => {
		...
		if (value && (value as string).length > 0) {
			if (capitalize) {
				...
				console.log('capitalized', value);
			}
			if (
				(exportValueOn as TExportValueOn).includes('keypress') ||
				((exportValueOn as TExportValueOn).includes('enter') && event.key === 'Enter')
			) {
				value = capitalize(value as string) ?? '';
				if (onInputIsReadyCallback) {
					onInputIsReadyCallback();
